 ...
 * +--------+------+------+
 * | offset | kind | term | --> offset 是对应的日志条目在日志文件中的偏移量
 * +--------+------+------+
 * | offset | kind | term |
 * +--------+------+------+
 * | offset | kind | term |
 * +--------+------+------+
 *
 * minEntryIndex -> 索引文件中第一条记录对应的日志条目在日志文件中的偏移量
 * maxEntryIndex -> 索引文件中最后一条记录对应的日志条目在日志文件中的偏移量
 * 由此可计算出索引文件包含的日志条目数 = maxEntryIndex - minEntryIndex + 1
 */
public class EntryIndexFile implements AutoCloseable, Iterable<EntryIndexItem> {
    ...
    /**
     * index -> EntryIndexItem, index 是对应的日志条目在日志文件中的索引
     */
    ...
    /**
     * 追加日志条目索引信息
     * 追加必须是顺序追加，也即当前这条追加的索引信息必须严格相邻于上一条索引信息
     */
