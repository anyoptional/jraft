...
public class DefaultNodeConfigLoader implements NodeConfigLoader {
    ...
    @Override
    public NodeConfig load(InputStream input) throws IOException {
        ...
        config.setMaxReplicationEntries(getIntProperty(p, "replication.entries.max", Integer.MAX_VALUE));
        config.setSnapshotDataLength(getIntProperty(p, "snapshot.data.length", 1024));
        config.setMaxReplicationEntriesForNewNode(getIntProperty(p, "new-node.replication.entries.max", Integer.MAX_VALUE));
